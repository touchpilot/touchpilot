...
import dev.touchpilot.app.workflow.WorkflowLivePolicyContext
...
class WorkflowReplayCoreTest {
    ...
    fun localReasoningCoreReplaysWorkflowThroughConfiguredEngine() {
        ...
        assertEquals(ToolSource.WORKFLOW_REPLAY, assertIs<AgentEvent.ToolRequested>(
            result.events.first { it is AgentEvent.ToolRequested }
        ).source)
    }
    ...
    private class FakeObserveTools : LocalAgentLoopTools {
        ...
        override fun execute(
            ...
            source: ToolSource,
            foregroundApp: dev.touchpilot.app.androidcontrol.ForegroundAppInfo,
