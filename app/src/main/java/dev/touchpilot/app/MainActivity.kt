...
class MainActivity : Activity() {
    ...
    private fun runAgentFromChat(task: String) {
        ...
        Thread {
            val runOutcome = runCatching { reasoningCore.run(agentTask) }
    ...
    private fun removeWorkingIndicator(workingIndex: Int) {
        if (workingIndex in conversation.indices &&
            conversation[workingIndex] is ChatEvent.Agent &&
            (conversation[workingIndex] as ChatEvent.Agent).text == "Working on it."
        ) {
